...
//   players/  — TTL 1h; avoids re-fetching match-ID list on every server restart
...
// ── Player disk cache (TTL 1h — contains recent match-ID list) ───────────────
...
// Serial queue: every API call chains onto the previous one so they run one at
// a time. Sliding-window check inside ensures we never exceed 10/min.
const RATE_LIMIT = 10;
const RATE_WINDOW = 60 * 1000;
// The sliding window above is a best-effort local guess. PUBG still returns 429
// when several consumers overlap (notifier scan + prewarm + ad-hoc requests),
// so a bounded retry keeps one 429 from dropping a whole member from a scan.
...
async function _doRateLimitedGet(apiPath) {
  ...
  const now = Date.now();
  while (_callTimes.length && now - _callTimes[0] > RATE_WINDOW) _callTimes.shift();

  if (_callTimes.length >= RATE_LIMIT) {
    const wait = RATE_WINDOW - (now - _callTimes[0]) + 100; // +100ms safety margin
    console.log(`[PUBG] Rate limit: queuing ${wait}ms (${_callTimes.length}/${RATE_LIMIT} calls in window)`);
  ...
  for (let attempt = 0; attempt <= MAX_429_RETRIES; attempt++) {
    try {
      ...
      await new Promise(r => setTimeout(r, wait));
      const n3 = Date.now();
      while (_callTimes.length && n3 - _callTimes[0] > RATE_WINDOW) _callTimes.shift();
      _callTimes.push(Date.now());
    }
  }
  throw lastErr;
    ...
    const req = https.request(options, res => {
      ...
      res.on('end', () => {
        if (res.statusCode === 429) {
          ...
          err.retryAfterMs = Number.isFinite(ra) ? Math.ceil(ra * 1000) + 250 : null;
          return reject(err);
        }
