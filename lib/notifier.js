...
//   • matchType + age filters — no custom-lobby false alerts, no restart floods
...
const { DATA, loadEnv }              = require('./config');
...
const SKIP_MATCH_TYPES = new Set(['custom', 'training', 'airoyale', 'event']);
...
function buildEmbed(playerName, achievements, stats, modeMap) {
  ...
  return {
    ...
    author:    { name: `[APES] ${playerName}` },
    footer:    { text: modeMap },
    timestamp: new Date().toISOString(),
  };
}

// Squad embed — dinner or near-miss, all APES members in one post.
...
function buildSquadEmbed(participants, modeMap, type) {
  ...
  const subtitle   = count > 1 ? `${placement}  ·  ${count} APES  ·  ${totalKills} kills` : `${placement}  ·  Solo`;

  return {
    title:       type === 'dinner' ? '🍗  CHICKEN DINNER!' : '😤  SO CLOSE',
    description: lines.join('\n'),
    color:       COLORS[type],
    author:      { name: `[APES]  ·  ${subtitle}` },
    footer:      { text: modeMap },
    timestamp:   new Date().toISOString(),
  };
}
...
async function scanMember(member, notified, records, webhookUrl, postsThisScan, dinnerQueue, nearMissQueue) {
  ...
  // Bootstrap
  if (!notified[member.accountId] || notified[member.accountId].length === 0) {
  ...
  if (newIds.length === 0) return postsThisScan;
  ...
  for (const matchId of newIds) {
    ...
    // Match type filter
    if (SKIP_MATCH_TYPES.has(attrs.matchType)) {
      logEvent(`⏭ Skip [${attrs.matchType}] — ${member.name}`);
      seen.add(matchId);
      continue;
    }
    ...
    const achievements = detectAchievements(stats, records, member.accountId);
...
async function postSquadQueue(queue, type, members, webhookUrl, postsThisScan, notified) {
  for (const [matchId, { matchData, modeMap, participants }] of queue) {
    ...
    try {
      ...
      // Mark this matchId as posted so late-arriving squad members don't trigger a second post
      if (notified) {
        if (!notified._posted) notified._posted = [];
        if (!notified._posted.includes(matchId)) notified._posted.push(matchId);
        notified._posted = notified._posted.slice(-500); // cap size
        saveJson(NOTIFIED_FILE, notified);
      }
    } catch (e) {
...
async function scan() {
  ...
  for (const member of members) {
    postsThisScan = await scanMember(
      member, notified, records, webhookUrl, postsThisScan, dinnerQueue, nearMissQueue
    ).catch(e => { logEvent(`⚠ Error scanning ${member.name}: ${e.message}`); return postsThisScan; });
...
function startNotifier() {
  const env        = loadEnv();
  const webhookUrl = env.DISCORD_TEST_WEBHOOK_URL || env.DISCORD_WEBHOOK_URL || null;
  if (!webhookUrl) console.log('[APES Notifier] ⚠ No webhook URL — will not post until configured');
  else if (env.DISCORD_TEST_WEBHOOK_URL) console.log('[APES Notifier] 🧪 TEST mode');
  else console.log('[APES Notifier] 🚀 LIVE — polling every 5 min');
