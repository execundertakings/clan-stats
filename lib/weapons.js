...
const RETIRING_SOON_WEAPONS = new Set([
  'WeapMosinNagant_C',
  'WeapR45_C',
  'WeapDP28_C',
  'WeapPP19_C',
...
function weaponStatus(internalName) {
  if (!internalName) return { retiringSoon: false };
  if (RETIRING_SOON_WEAPONS.has(internalName)) {
    return {
      retiringSoon: true,
      label: 'Scheduled for removal in Update 42.1',
      shortLabel: '42.1 exit',
    };
  }
  return { retiringSoon: false };
}
...
module.exports = { WEAPON_NAMES, weaponDisplayName, weaponStatus, isIgnoredCauser };
