# 3PI Clan Stats — Project Context for Codex

## What this is

A self-hosted PUBG clan stats website for the **3PI** (Third Party Incorporated) clan on PC. Pulls data from the official PUBG API and displays it in a dark-themed, interactive single-page app. Runs locally at **localhost:3002**. Exposed publicly at **https://3pi.executiveundertakings.com** via Cloudflare tunnel.
...
**Rule:** Every AI- or LLM-driven feature in this project is implemented through a **Cowork scheduled task**. Codex does the synthesis when the task runs. There are **no direct calls to api.anthropic.com**, no `ANTHROPIC_API_KEY` in `.env`, no `lib/anthropic.js`-style HTTP clients.
...
2. **A Cowork scheduled task** (configured in the Cowork UI, with its instructions kept under `scheduled-tasks/*.skill.md` for version control) runs the prep script, reads the prepared input, performs the LLM synthesis as Codex executing the task, and writes the output JSON to `data/`.
...
When building `match_history_cache.json`, two hard filters apply to every match:
- Skip if `matchType !== 'official'` (removes casual/arcade/custom games)
- Skip if `gameMode` doesn't include `'squad'` (removes solo/duo)

Only matches passing both filters are aggregated. This script is the **only** writer of `match_history_cache.json`.
...
**Deprecated (no longer consumed by frontend):** `/api/trends`, `/api/analysis` — these read from `stats_cache.json` (raw API) and lack the multi-mode fallback. Trends and analysis are now computed client-side from `resolvedStats`.
